######## R functions that are potentially helpful
...
updateBF_matern=function(d, D,nIndx,nnIndxLU, CIndx, n, sigmaSq, tausq, phi, nu)#int covModel
{
  ...
  for(i in 2:n)
  {
    ...
    for (k in 1:nnIndxLU[n+i])
    {
      ...
      if (dd > 0)
      {
        c[nnIndxLU[i]+k] = sigmaSq*Matern(d=dd,alpha=phi,nu=nu) #not the same as the materncpp
      }
      for(l in 1:nnIndxLU[n+i])
      {
        ddd=D[CIndx[i]+(l-1)*nnIndxLU[n+i]+k]
...
##### select knots:
selectknotnew=function(tem,splitnum)
{
  coor=tem[,c("x","y")]
  xcoor=sort(unique(coor[,"x"]))
  ycoor=sort(unique(coor[,"y"]))
  xwidth=floor((length(xcoor)-1)/splitnum)
  knots.x=xcoor[seq(1,length(xcoor),by=xwidth)]
  ywidth=floor((length(ycoor)-1)/splitnum)
  knots.y=ycoor[seq(1,length(ycoor),by=ywidth)]
  Knots=numeric()
  knot=1
  for (a in 1:nrow(tem))
...
lrmh=function(n, m, wstar, kij, lambda, ci, invCistar) #checked
{
  output=numeric()
  i = 1;
  cij = ci[,i]
  me=lambda[i] + t(cij) %*% invCistar %*% t(wstar) #wstar is a 1 by m matrix!
  output[i] = dnorm(kij[i],me,1);
  for (i in 2:n){
    cij = ci[,i]
    me=lambda[i] + t(cij) %*% invCistar %*% t(wstar)
    output[i] = dnorm(kij[i],me,1)
  }
  return(output)
}

######## Transform a vector into matrix:
#v=round(c(V[1,1],V[1,2],V[1,3],V[1,4],V[1,5],V[1,6],V[1,7],V[2,2],V[2,3],V[2,4],V[2,5],
#        V[2,6],V[2,7],V[3,3],V[3,4],V[3,5],V[3,6],V[3,7],V[4,4],V[4,5],V[4,6],V[4,7],
#        V[5,5],V[5,6],V[5,7],V[6,6],V[6,7],V[7,7]),7)
vectomat=function(v,N) #checked
{
  Vmat=matrix(NA,N,N)
  index.v=1
  for (i in 1:N)
...
######## Transform a matrix into a vector of length (n+1)n/2:
mattovec=function(V,N) #checked
{
  Vvec=numeric()
  index.v=1
  for (i in 1:N)
...
####### sl evaluation part:
slupdate=function(ii,kij,whole.index,pstar,cancerwhole,cancer)
{
  kijtest=kij[whole.index[[ii]]] 
  pstartest=pstar[whole.index[[ii]]] 
  cancerwhole[[ii]][s,]=svccancerupdatecpp(kijtest, pstartest)
  ### important:
  cancer[whole.index[[ii]]]=cancerwhole[[ii]][s,]
  return(list(cancerwhole=cancerwhole,cancer=cancer))
}

####### Evaluation Table:
evaluation_table=function(true_category,pred_category,Levels)
{
  true_category=as.factor(true_category)
  Z=length(Levels)
  evaltable=matrix(NA,Z,Z)
  rownames(evaltable)=paste("True=",Levels,sep="")
  colnames(evaltable)=paste("Pred=",Levels,sep="")
  gridindex=expand.grid(x=Levels,y=Levels)
  for (x in 1:nrow(gridindex))
  {
    evaltable[gridindex[x,1],gridindex[x,2]]=sum((true_category==(as.numeric(as.character(gridindex[x,1]))))&(pred_category==(as.numeric(as.character(gridindex[x,2])))))
  }
  return(evaltable)
}
